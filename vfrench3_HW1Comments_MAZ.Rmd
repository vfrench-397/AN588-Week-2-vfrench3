---
title: "vfrench3_FinalHomeworkCode_01"
...
```{r}
#subset the vector v using seq function. seq() generates regular interval sequences. In this case we are starting at element 0 of v to the final element of v and taking every 3rd element. length() generates the length of the argued vector. 
...
```{r}
#since the elements of the list currently are titled 1 and 2, it will be easier to read later if we name them as the lineages. We do this by assigning a list consisting of the character values Platyrrhini and Cattarhini to the function names with the argument being our list object Anthropoidea 
...
*Great work, and great annotating! -Mel*
